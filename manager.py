...
class Manager(QThread):
    ...
    def findRMS(self, search_mode, frequency_mode):
        ...
        for x in np.arange(self.freq_lowlimit_hz, self.freq_highlimit_hz, self.step):
            ...
            self.times_s, self.voltages_v = self.Oscilloscope.capture(
                1)  # populates times_s and voltages_v with set frequency
        ...
        self.profile_plot_signal.emit(self.list_of_frequencies,
                                      self.list_of_rms_values)  # frequencies will be on the x-axis
