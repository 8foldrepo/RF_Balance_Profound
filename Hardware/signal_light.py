...
def main():
    """
    Use the command line to control the light directly with integer arguments for example "python signal_light.py 1 0 1"
    (calls the set light command (1) to turn the red light (0) on (1))
    first arg n: command ID (1: set_light 2: set tower, 3: set_buzz, 4: set_buzzer_ex, 5: reset)
    The remaining arguments are passed to the method of the same name, refer to their docstrings.
    """
    args = sys.argv
    argc = len(sys.argv)
...
def open_light(clearIn=True, clearOut=True):
    """
    Open USB controlled stacked signal light
...
def set_tower(dev: Device, red: int, yellow: int, green: int, blue: int, white: int) -> bool:
    ...
    dev: Device
        Device instance to be sent
    red: int
        Red LED pattern (off: 0, on: 0x1, LED pattern 1: 0x2, LED pattern 2: 0x3, LED pattern 3: 0x4, LED pattern 4:
        0x5, keep current setting: 0x6 to 0xF)
    yellow: int
        Yellow LED pattern (off: 0, on: 0x1, LED pattern 1: 0x2, LED pattern 2: 0x3, LED pattern 3: 0x4, LED pattern 4:
        0x5, keep current setting: 0x6 to 0xF)
    green: int
        Green LED pattern (off: 0, on: 0x1, LED pattern 1: 0x2, LED pattern 2: 0x3, LED pattern 3: 0x4, LED pattern 4:
        0x5, keep current setting: 0x6 to 0xF)
    blue: int
        Blue LED pattern (off: 0, on: 0x1, LED pattern 1: 0x2, LED pattern 2: 0x3, LED pattern 3: 0x4, LED pattern 4:
        0x5, keep current setting: 0x6 to 0xF)
    white: int
        White LED pattern (off: 0, on: 0x1, LED pattern 1: 0x2, LED pattern 2: 0x3, LED pattern 3: 0x4, LED pattern 4:
        0x5, keep current setting: 0x6 to 0xF)
